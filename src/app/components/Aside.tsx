...
export default function Aside() {
  const SIDEBAR_ARCHIVES = [
    { icon: <FolderOpen size={13} />, label: "Global Archive", href: "/dashboard/", active: true },
    { icon: <FileText size={13} />, label: "Heritage Docs", href: "/dashboard/heritage", active: false },
    { icon: <Globe size={13} />, label: "Public Node", href: "/nodes/public", active: false },
    { icon: <Settings size={13} />, label: "Node Settings", href: "/nodes/settings", active: false },
  ];
  return (

    <aside
      className="flex-shrink-0 flex flex-col justify-between border-r py-5 px-4"
    ...
    >
      <div>
        ...
        <div className="flex flex-col gap-0.5">
          {SIDEBAR_ARCHIVES.map(({ icon, label, href, active }) => (
            <Link
              key={label}
              href={href}
